...
class Authenticator:
    ...
    def __init__(self):
        ...
        self.auth_private_key = self.auth_key.exportKey("PEM")
        self.auth_public_key = self.auth_key.publickey().exportKey("PEM")
        self.auth_cipher = PKCS1_OAEP.new(self.auth_key)

    def getKey(self):
        print("")
    def sendKey(self):
        print()
    def authenticate(self,messages):
        encryptedVoteList = ""
        deliminator = ",,,,,".encode("utf-8")
        for message in messages:
            plainMessage = self.auth_cipher.decrypt(message)
            ...
            if result[0] == "True":
                ...
                if(encryptedVoteList ==""):
                    encryptedVoteList = cipherVote_2
                else:
                    encryptedVoteList = encryptedVoteList + deliminator + cipherVote_2
                continue
        create_Signature(encryptedVoteList)
        create_Encrypted_Ballot(encryptedVoteList)
